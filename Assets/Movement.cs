...
public class Movement : MonoBehaviour
{
    ...
    [SerializeField]
    private float maxSpeed = 10f;      // Maximum player speed on the ground
    [SerializeField]
    private float maxAirSpeed = 0.6f;   // "Maximum" player speed in the air
    [SerializeField]
    private float friction = 10f;        // How fast the player decelerates on the ground
    [SerializeField]
    ...
    private bool CheckGround()
    {
        ...
        bool result = Physics.Raycast(ray, GetComponent<Collider>().bounds.extents.y + 0.3f, groundLayers);
        return result;
    }
}